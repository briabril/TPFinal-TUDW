  ...
  return (
    <Box sx={{ minHeight: "100vh", display: "flex", flexDirection: "row", justifyContent: "flex-start", alignItems: "flex-start", width: "100%" }}>
      ...
      <Box sx={{ flex: 1, p: 4 }}>
        ...
        {/* Tabs: en modo oscuro la pestaña seleccionada usa el celeste claro de la paleta (--info) y es más bold */}
        {(() => {
          const theme = useTheme();
          const isDark = theme.palette.mode === "dark";
          const selectedColor = isDark ? "var(--info)" : undefined;
          const selectedSx = {
            "&.Mui-selected": {
              color: selectedColor ?? theme.palette.primary.main,
              fontWeight: 700,
            },
          };
          ...
          return (
            <Tabs value={tab} onChange={(e, v) => setTab(v)} sx={{ mb: 3 }}>
              <Tab label="Likes" sx={selectedSx} />
              <Tab label="Reposts" sx={selectedSx} />
              <Tab label="Comentarios" sx={selectedSx} />
            </Tabs>
          );
        })()}
              ...
              <Card key={post.id} sx={{ mb: 2 }}>
                <CardContent sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
                  <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                    <Avatar src={post.post_author_avatar || "/default-avatar.png"} alt={post.author_username} sx={{ width: 40, height: 40 }} />
        ...
        {tab === 1 && (
          <Box>
            {sharedPosts.length === 0 && <Typography color="text.secondary">No hiciste reposts aún.</Typography>}
          ...
          <Box>
            {myComments.map((comment: any) => (
              <Card key={comment.id} sx={{ mb: 2 }}>
                <CardContent sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
                  <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                    <Avatar src={comment.post_author_avatar || "/default-avatar.png"} alt={comment.post_author} sx={{ width: 40, height: 40 }} />
                    <Typography variant="subtitle1" fontWeight={600}>
                      {comment.post_author}
                    </Typography>
                  </Box>

                  <Typography sx={{ ml: 5 }}>{comment.content}</Typography>
                </CardContent>
              </Card>
            ))}
          </Box>
